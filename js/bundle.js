...
function calculator() {
  ...
  function getStaticInformation(parentSelector, activeClass) {
    var element = document.querySelectorAll("".concat(parentSelector, " div"));
...
function forms(formSelector, modalTimerId) {
  ...
  function showThanksModal(message) {
    var prevModalDialg = document.querySelector('.modal__dialog');
    prevModalDialg.classList.add('hide');
    (0,_modal__WEBPACK_IMPORTED_MODULE_9__.openModal)('.modal', modalTimerId); // откривает модальное окно когда функція визивается 
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      //окно будет появляться не доходя до последнего пікселя монітора(-1);
      openModal(modalSelector, modalTimerId);
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    // 0 - по умолчанію ставітся первий обьект у список ES6
...
function timer(id, deadLine) {
  ...
  function getTimeRemainig(endtime) {
    ...
    var t = Date.parse(endtime) - Date.parse(new Date()); //разніца между дедлайном і текущей датой (в міллісекундах)
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  }, 300000); //откривает модальное окно чз 5 сек после того как юзер зашел на сайт 
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2023-12-28');
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_8__["default"])('form', modalTimerId);
